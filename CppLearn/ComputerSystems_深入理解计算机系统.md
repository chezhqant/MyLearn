...
$$
8. 计算机负数表示
> 计算机最常见的有符号数的计算机表示方式就是补码，在这个定义中，将字的最高有效位解释为负权，用B2T\_{\omega}表示：
$$
B2T\_{\omega}=-x\_{{\omega}2^{\omega-1}}+\sum\_{\omega-2}^{i=0} {x\_{i}2^{i}} \tag{3}\label{eg:3}
$$
最高有效位x\_{\omega-1}也称为符号位，它的权重为-2^{\omega-1}，符号位被设置为1时，表示值为负，而当设置为0时，值为非负。举个例子：
$$
\begin{split}
