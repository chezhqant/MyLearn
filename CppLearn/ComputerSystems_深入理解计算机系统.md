...
将一个$\omega$位的数$\vec{[x\_{\omega-1}, x\_{\omega-2}, \ldots, x\_{0}]}$截断为一个k为数字时，我们会丢弃高$\omega-k$位，得到一个位向量\vec{[x\_{k-1}, x\_{k-2}, \ldots, x\_{0}]}。截断一个数字可能会改变它的值--溢出的一种形式。对于一个无符号数字x，截断它到k为得结果就相当于计算$x mod(2^{k})$。  
$$
\begin{split}
...
在这段推导中，我们利用的属性就是：对于任何i$\ge$k,$2^{i}$mod$2^{k}$=0和\sum\_{i=0}^{k-1} {x\_{i}2^{i}}$\le$\sum\_{i=0}^{k-1} {2^{i}}=2^{k}-1$\lt$2^{k}。   
